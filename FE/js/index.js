...
function getUserData() {
    axios({
        ...
        showUsersData(user.friendsList)
    }).catch(function (error) {
        console.log(error);
    });
}
...
function showUsersData(users = []) {
    ...
    for (let i = 0; i < users.length; i++) {
        cartonna += `
        <div onclick="displayChatUser('${users[i].userId._id}')" class="chatUser my-2">
        <img class="chatImage" src="${users[i].userId.image?.secure_url || avatar}" alt="" srcset="">
        <span class="ps-2">${users[i].userId.username}</span>
    </div>
